...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="card full-height">
            <div class="card-body">
                <form action="{{ url('/admin/quotation/archive/'.$year) }}">
                    @csrf
                    ...
                    <table id="basic-datatables" class="display table table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $pro->quotation->quotationNo }}</td>
                                <td>{{ $pro->quotation->perusahaan->nama }}</td>
                                <td>{{ $pro->nama }}</td>
                                {{-- <td>{{ Carbon\Carbon::parse($qto->tglQuotation)->format('d-M-Y') }}</td> --}}
                                <td>
                                    <a href="{{ route('admin.project.ongoing.edit',$pro->id) }}" title="Quotation" class="btn btn-info"><i class="fas fa-file-invoice"></i></a>
                                    <a href="{{ route('admin.project.unarchive',$pro->id) }}" title="Unarchive" class="btn btn-secondary"><i class="fas fa-archive"></i></a>
                                </td>
                            </tr>
                            @endforeach
